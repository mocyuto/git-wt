...
- `tmux open`: 指定したワークツリーの tmux ウィンドウを開く、またはアクティベートします。ウィンドウが存在すれば切り替え、存在しなければ設定に従って作成します。
- `tmux save [session-name]`: 現在の tmux ウィンドウの状態を保存します。`zgt` で開かれたウィンドウかどうかも記録されます。
- `tmux restore [session-name]`: 保存された状態から tmux ウィンドウを復旧します。`zgt` で開かれたウィンドウは、その設定（ペイン分割など）を含めて復旧されます。
