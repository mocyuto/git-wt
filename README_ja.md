...
- `tmux ls`: 使用可能な tmux セッション、ウィンドウ、ペインの状態（Running/Waiting）を階層構造（ツリー形式）で表示します。
- `tmux open`: 指定したワークツリーの tmux ウィンドウを開くか、アクティブにします。ウィンドウが既に存在すればそこに切り替わり、存在しなければ設定に従って作成します。
