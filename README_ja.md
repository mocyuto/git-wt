...
6. **既存ワークツリーの検知**: すでにワークツリーが存在するブランチを追加しようとした場合、エラーにするのではなく、そのワークツリーに対してフックや事後処理を実行するかどうかを確認します。
...
- `config`: マージされた最終的な設定（グローバル + プロジェクト + フラグ）をYAML形式で表示します。`--check` フラグを使うことで、設定ファイルの構文エラーをチェックできます。
- `config edit`: システムエディタを使用して設定ファイルを編集します。
