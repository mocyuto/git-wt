...
- Go 言語による単一バイナリでの動作
- ブランチ名に基づくパスの自動生成（リポジトリルートと同じ階層に `{プロジェクト名}-{ブランチ名}`）。カスタムパスを指定するには `--path` を使用。
...
- `tmux open`: 指定したワークツリーの tmux ウィンドウを開くか、アクティブにします。
- `tmux close`: 指定したワークツリーの tmux ウィンドウを安全に（SIGTERM 送信後に待機して）閉じます。
...
tmux:
  enabled: true
  auto_close: true # tmux ウィンドウを自動的かつ安全に閉じる (デフォルト: true)
  panes:
    - id: main
      commands: ["yarn"]
    - id: dev
...
| `auto_close`  | `zgt rm` 実行時にウィンドウを自動的かつ安全に閉じるかどうか (デフォルト: `true`)。 |
